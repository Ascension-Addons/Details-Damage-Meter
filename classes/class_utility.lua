				...
				if (targetGUID and not checked[targetGUID]) then
					...
					if (rect and rect <= 4) then
						...
						for debuffIndex = 1, 41 do
							local auraName, _, _, _, _, _, _, unitCaster, _, _, spellId = UnitAura (target, debuffIndex, 'HARMFUL')
							if (auraName) then
			...
			for raidIndex = 1, GetNumGroupMembers()-1 do
				...
				if (his_target and not checked[his_target] and rect and rect <= 4) then
					...
					for debuffIndex = 1, 40 do
						local auraName, _, _, _, _, _, _, unitCaster, _, _, spellId = UnitAura ("party"..raidIndex.."target", debuffIndex, 'HARMFUL')
						if (auraName and unitCaster) then
							local playerName, realmName = _UnitName (unitCaster)
							local playerGUID = UnitGUID(unitCaster)
							if (playerGUID) then
				...
				for debuffIndex = 1, 40 do
					...
					if (name and unitCaster) then
						...
						if (playerGUID) then
							...
							Details.parser:add_debuff_uptime (nil, GetTime(), playerGUID, playerName, 0x00000417, his_target, _UnitName ("playertarget"), 0x842, nil, spellid, name, sOperationType)
						end
					end
				end
			end
